...
# ==============================================================================
"""Joystick task for HopeJR. (based on Berkeley Humanoid)"""
...
from . import base as hopejr_base

# from playground.common.utils import LowPassActionFilter
from playground.new_hopejr.retarget_reference_motion import RetargetReferenceMotion
...
class JoystickRetarget(hopejr_base.HopeJREnv):
    ...
    def _post_init(self) -> None:
        ...
        if USE_IMITATION_REWARD:
            self.PRM = RetargetReferenceMotion(
                "playground/new_hopejr/data/humanoid3d_walk.txt"
            )
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        if USE_IMITATION_REWARD:
            current_reference_motion = self.PRM.sample_frame(0)
        else:
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        if USE_IMITATION_REWARD:
            state.info["current_reference_motion"] = self.PRM.sample_frame(
                state.info["imitation_i"]
            )
        else:
